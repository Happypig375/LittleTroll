Structure SurrogatePair
    ...
    Public Function Asc() As UInteger
        Return If(IsSurrogatePair, &H10000UI + CUInt(AscW(HighSurrogate) - &HD800UI) *
            &H400UI + (CUInt(AscW(LowSurrogate)) - &HDC00UI), CUInt(AscW(SingleChar)))
    End Function
    Public Shared Function Asc(Pair As SurrogatePair) As UInteger
        Return If(Pair.IsSurrogatePair, &H10000UI + CUInt(AscW(Pair.HighSurrogate) - &HD800UI) *
            &H400UI + (CUInt(AscW(Pair.LowSurrogate)) - &HDC00UI), CUInt(AscW(Pair.SingleChar)))
    End Function
    ...
    Sub New(CodePoint As UInteger)
        ...
        ElseIf CodePoint <= &H10FFFF Then
            IsSurrogatePair = True
            CodePoint -= &H10000UI
            LowSurrogate = ChrW(CInt(CodePoint Mod &H400UI + &HDC00UI))
            HighSurrogate = ChrW(CInt(CodePoint \ &H400UI + &HD800UI))
        Else
